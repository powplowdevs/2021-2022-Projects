...
def create_grid():
    ...
    #         block_state = random.randint(0,6)
    #         if block_state < 2:
    #             grid[y][x] = 3
...
def path_find():
    global ended, grid, open_grids, closed_grids, start, target_spot, side_options, side_av, sos
    while not ended:
        f_values = []
        xys = []
        for i in range(8):
            try:
                ...
                if i == 0:
                    ...
                    distance += G
                    if grid[spot[0]][spot[1]] == 3 or [spot[0],spot[1]] in closed_grids:
                        distance += 99999
                    if spot == target_spot:
                        ended = True
                        break
        ...
        if not side_av:
            for index, value in enumerate(f_values):
                if value == min(f_values) and index != current:
                    ...
                    sos = [xys[current][0], xys[current][1]]
                    side_av = True
                    print("same")
                    pg.time.wait(1000)
        if side_av:
            if side_options[0][1] < min(f_values):
                ...
                print("yes")
                grid[sos[0]][sos[1]] = 6
                closed_grids.append([sos[0],sos[1]])
                start = [sos[0],sos[1]]
                sos = []
                side_av = False
                pg.time.wait(1000)
